...
Class Master extends DBConnection {
	...
	function save_product(){
		foreach($_POST as $k =>$v){
			if(!is_array($_POST[$k]) && !is_numeric($v)){
				$_POST[$k] = $this->conn->real_escape_string($v);
			}elseif(is_array($_POST[$k])){
				foreach($_POST[$k] as $kk => $vv){
					if(!is_numeric($vv))
					$_POST[$k][$kk] = $this->conn->real_escape_string($vv);
				}
			}
		}
		extract($_POST);
		...
		if($save){
			...
			if(!isset($resp)){
				$data2 = "";
				$err="";
				$this->conn->query("DELETE FROM `specification_list` where product_id = '{$pid}' ");
				if(isset($specs)){
	...
	// --- BẮT ĐẦU SỬA HÀM PLACE_ORDER ---
	function place_order(){
		extract($_POST);
		$client_id = $this->settings->userdata('id');
		
		// --- BẮT ĐẦU CODE MỚI ---
		// Khởi tạo biến khuyến mãi
		$discount_amount = 0;
		$promotion_id = "NULL"; // Dùng "NULL" cho SQL

		// Kiểm tra xem có mã hợp lệ trong session không
		if(session_id() == "") session_start(); // Đảm bảo session đã bắt đầu
		if(isset($_SESSION['promo_id']) && isset($_SESSION['promo_discount'])){
			// Lấy giá trị đã tính toán từ apply_promotion
			// Chúng ta tin tưởng $amount từ form (đã được JS cập nhật)
			$discount_amount = $_SESSION['promo_discount'];
			$promotion_id = $_SESSION['promo_id'];
			
			// Tăng số lượt sử dụng mã
			$this->conn->query("UPDATE `promotions` SET `uses_count` = `uses_count` + 1 WHERE `id` = {$promotion_id}");
		}
		// --- KẾT THÚC CODE MỚI ---
		...
		$data .= " ,amount = '{$amount}' "; // $amount này đã được JS cập nhật (tổng - giảm giá)
		$data .= " ,paid = '{$paid}' ";
		...
		$order_sql = "INSERT INTO `orders` set $data"; // Giữ nguyên cú pháp SET
		...
		if($save_order){
			...
			$data = '';
			$cart = $this->conn->query("SELECT c.*,p.name,p.price,p.id as pid from `cart` c inner join `inventory` i on i.id=c.inventory_id inner join products p on p.id = i.product_id where c.client_id ='{$client_id}' ");
	...
	// --- BẮT ĐẦU HÀM MỚI: APPLY_PROMOTION ---
	function apply_promotion(){
        // Bắt đầu session để lưu mã
        if(session_id() == "") session_start();
        
        // Xóa mã cũ nếu người dùng thử mã mới
        unset($_SESSION['promo_id']);
        unset($_SESSION['promo_discount']);
        unset($_SESSION['promo_code']);

        // Lấy mã code từ form
        $code = $this->conn->real_escape_string($_POST['promo_code']);
        $client_id = $_settings->userdata('id');

        // 1. Kiểm tra mã
        $qry = $this->conn->query("SELECT * FROM `promotions` WHERE `code` = '{$code}'");
        if(!$qry || $qry->num_rows <= 0){
            return json_encode(['status' => 'error', 'msg' => 'Invalid promotion code.']);
        }
        $promo = $qry->fetch_assoc();
        $promo_id = $promo['id'];

        // 2. Validate mã
        if($promo['status'] == 0){
            return json_encode(['status' => 'error', 'msg' => 'This promotion is inactive.']);
        }
        if($promo['uses_count'] >= $promo['max_uses']){
            return json_encode(['status' => 'error', 'msg' => 'This promotion has reached its usage limit.']);
        }
        $now = date('Y-m-d H:i:s');
        if(strtotime($now) < strtotime($promo['start_date'])){
            return json_encode(['status' => 'error', 'msg' => 'This promotion has not started yet.']);
        }
        if(strtotime($now) > strtotime($promo['end_date'])){
            return json_encode(['status' => 'error', 'msg' => 'This promotion has expired.']);
        }

        // 3. Tính toán Subtotal (tổng tiền giỏ hàng)
        $cart_total = 0;
        $cart = $this->conn->query("SELECT c.*, p.price 
                                   FROM `cart` c 
        ...
        if($promo['type'] == 'percent'){
            ...
            if($discount_amount > $cart_total){
                $discount_amount = $cart_total; // Không cho giảm giá nhiều hơn tổng tiền
            }
        }

        // 5. Lưu vào Session để dùng khi "Place Order"
        $_SESSION['promo_id'] = $promo_id;
        $_SESSION['promo_discount'] = $discount_amount;
        $_SESSION['promo_code'] = $code;

        $resp['status'] = 'success';
        $resp['msg'] = "Code '{$code}' applied! You saved $" . number_format($discount_amount, 2);
        $resp['discount_amount'] = number_format($discount_amount, 2);
        
        return json_encode($resp);
    }
	// --- KẾT THÚC HÀM MỚI ---

} // <-- ĐÂY LÀ DẤU NGOẶC KẾT THÚC CLASS MASTER
...
switch ($action) {
	...
	// --- BẮT ĐẦU THÊM CASE MỚI ---
	case 'apply_promotion':
		echo $Master->apply_promotion();
	break;
	// --- KẾT THÚC THÊM CASE MỚI ---
