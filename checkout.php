                        ...
                        <div class="col">
                            ...
                            <div class="card-footer bg-white">
                                <form id="promo-form">
                                    <div class="input-group">
                                        <input type="text" class="form-control" name="promo_code" id="promo_code" placeholder="Enter Promotion Code">
                            ...
                            <ul class="list-group list-group-flush">
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    <span>Subtotal</span>
                                    <strong id="subtotal">$<?php echo number_format($total, 2) ?></strong>
                                </li>
                                ...
                                <li class="list-group-item d-flex justify-content-between align-items-center">
                                    ...
                                    <strong id="grand-total">$<?php echo number_format($total, 2) ?></strong>
                                </li>
                            </ul>
                        </div>
                        <hr>
                ...
                transactions: [
                    {
                    	//total purchase
                        amount: { 
                            // Sửa: Lấy tổng tiền từ #grand-total và đổi 'PHP' -> 'USD'
                        	total: $('#grand-total').text().replace('$', '').replace(/,/g, ''), 
                        	currency: 'USD' 
                        }
                    }
                ]
...
$(function(){
    ...
    // --- BẮT ĐẦU JS CHO MÃ GIẢM GIÁ ---
    $('#promo-form').submit(function(e){
        e.preventDefault();
        start_loader();
        
        $.ajax({
            url: 'classes/Master.php?f=apply_promotion',
            method: 'POST',
            data: $(this).serialize(),
            dataType: 'json',
            error: err => {
            ...
            success: function(resp){
                if(resp.status == 'success'){
                    // Lấy giá trị từ HTML
                    let subtotalText = $('#subtotal').text().replace('$', '').replace(/,/g, '');
                    let subtotal = parseFloat(subtotalText);
                    
                    // Tính toán
                    let discount = parseFloat(resp.discount_amount);
                    let grand_total = subtotal - discount;

                    // Cập nhật giao diện
                    $('#discount-amount').text('- $' + discount.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                    $('#discount-row').show();
                    $('#grand-total').text('$' + grand_total.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }));
                    
                    // Cập nhật input ẩn cho "place_order"
